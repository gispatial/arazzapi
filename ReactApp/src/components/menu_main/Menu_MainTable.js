...
import getColumns from './Menu_MainColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMenu_Main, getMenu_Main} from "../../repo/menu_mainRepo";
/*
...
const Menu_MainTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteMenu_Main(oldData.main_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Menu_Main Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getMenu_Main(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/menu_main/update/${rowData.main_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Menu_MainTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Menu_MainColumns';
