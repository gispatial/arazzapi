...
import getColumns from './Content_MgmtColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteContent_Mgmt, getContent_Mgmt} from "../../repo/content_mgmtRepo";
/*
...
const Content_MgmtTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteContent_Mgmt(oldData.id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Content_Mgmt Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getContent_Mgmt(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/content_mgmt/update/${rowData.id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Content_MgmtTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Content_MgmtColumns';
