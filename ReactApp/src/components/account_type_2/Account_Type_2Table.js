...
import getColumns from './Account_Type_2Columns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteAccount_Type_2, getAccount_Type_2} from "../../repo/account_type_2Repo";
/*
...
const Account_Type_2Table = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteAccount_Type_2(oldData.id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Account_Type_2 Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getAccount_Type_2(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/account_type_2/update/${rowData.id}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/account_type_2/add")
                }
            }
        ]}
...
export default withRouter(Account_Type_2Table);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Account_Type_2Columns';
