...
import getColumns from './Message_BoxColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMessage_Box, getMessage_Box} from "../../repo/message_boxRepo";
/*
...
const Message_BoxTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteMessage_Box(oldData.message_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Message_Box Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getMessage_Box(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/message_box/update/${rowData.message_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Message_BoxTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Message_BoxColumns';
