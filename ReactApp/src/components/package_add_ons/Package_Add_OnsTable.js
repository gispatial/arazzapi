...
import getColumns from './Package_Add_OnsColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePackage_Add_Ons, getPackage_Add_Ons} from "../../repo/package_add_onsRepo";
/*
...
const Package_Add_OnsTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePackage_Add_Ons(oldData.package_code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Package_Add_Ons Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPackage_Add_Ons(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/package_add_ons/update/${rowData.package_code}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/package_add_ons/add")
                }
            }
        ]}
...
export default withRouter(Package_Add_OnsTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Package_Add_OnsColumns';
