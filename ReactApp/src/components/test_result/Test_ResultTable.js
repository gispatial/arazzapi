...
import getColumns from './Test_ResultColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteTest_Result, getTest_Result} from "../../repo/test_resultRepo";
/*
...
const Test_ResultTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteTest_Result(oldData.patient_ic_no)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Test_Result Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getTest_Result(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/test_result/update/${rowData.patient_ic_no}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Test_ResultTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Test_ResultColumns';
