...
import getColumns from './Person_TypeColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePerson_Type, getPerson_Type} from "../../repo/person_typeRepo";
/*
...
const Person_TypeTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePerson_Type(oldData.id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Person_Type Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPerson_Type(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/person_type/update/${rowData.id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Person_TypeTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Person_TypeColumns';
