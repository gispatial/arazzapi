...
import getColumns from './Patient_BakColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePatient_Bak, getPatient_Bak} from "../../repo/patient_bakRepo";
/*
...
const Patient_BakTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePatient_Bak(oldData.refno)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Patient_Bak Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPatient_Bak(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/patient_bak/update/${rowData.refno}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Patient_BakTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Patient_BakColumns';
