...
import getColumns from './Six_Digit_CodeColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteSix_Digit_Code, getSix_Digit_Code} from "../../repo/six_digit_codeRepo";
/*
...
const Six_Digit_CodeTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteSix_Digit_Code(oldData.mobile)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Six_Digit_Code Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getSix_Digit_Code(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/six_digit_code/update/${rowData.mobile}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/six_digit_code/add")
                }
            }
        ]}
...
export default withRouter(Six_Digit_CodeTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Six_Digit_CodeColumns';
