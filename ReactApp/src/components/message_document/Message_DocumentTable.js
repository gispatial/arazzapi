...
import getColumns from './Message_DocumentColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMessage_Document, getMessage_Document} from "../../repo/message_documentRepo";
/*
...
const Message_DocumentTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteMessage_Document(oldData.message_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Message_Document Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getMessage_Document(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/message_document/update/${rowData.message_id}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/message_document/add")
                }
            }
        ]}
...
export default withRouter(Message_DocumentTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Message_DocumentColumns';
