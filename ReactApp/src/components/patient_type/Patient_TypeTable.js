...
import getColumns from './Patient_TypeColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePatient_Type, getPatient_Type} from "../../repo/patient_typeRepo";
/*
...
const Patient_TypeTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePatient_Type(oldData.patient_type_code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Patient_Type Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPatient_Type(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/patient_type/update/${rowData.patient_type_code}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Patient_TypeTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Patient_TypeColumns';
