...
import getColumns from './Add_On_ServicesColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteAdd_On_Services, getAdd_On_Services} from "../../repo/add_on_servicesRepo";
/*
...
const Add_On_ServicesTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteAdd_On_Services(oldData.add_on_code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Add_On_Services Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getAdd_On_Services(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/add_on_services/update/${rowData.add_on_code}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/add_on_services/add")
                }
            }
        ]}
...
export default withRouter(Add_On_ServicesTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Add_On_ServicesColumns';
