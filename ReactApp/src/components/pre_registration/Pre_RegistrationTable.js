...
import getColumns from './Pre_RegistrationColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deletePre_Registration, getPre_Registration} from "../../repo/pre_registrationRepo";
/*
...
const Pre_RegistrationTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deletePre_Registration(oldData.seq_reg_no)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Pre_Registration Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getPre_Registration(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/pre_registration/update/${rowData.seq_reg_no}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/pre_registration/add")
                }
            }
        ]}
...
export default withRouter(Pre_RegistrationTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Pre_RegistrationColumns';
