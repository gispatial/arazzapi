...
import getColumns from './Message_InboxColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMessage_Inbox, getMessage_Inbox} from "../../repo/message_inboxRepo";
/*
...
const Message_InboxTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteMessage_Inbox(oldData.message_inbox_code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Message_Inbox Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getMessage_Inbox(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/message_inbox/update/${rowData.message_inbox_code}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/message_inbox/add")
                }
            }
        ]}
...
export default withRouter(Message_InboxTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Message_InboxColumns';
