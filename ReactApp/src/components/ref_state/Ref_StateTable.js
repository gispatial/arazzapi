...
import getColumns from './Ref_StateColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteRef_State, getRef_State} from "../../repo/ref_stateRepo";
/*
...
const Ref_StateTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteRef_State(oldData.state_code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Ref_State Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getRef_State(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/ref_state/update/${rowData.state_code}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Ref_StateTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Ref_StateColumns';
