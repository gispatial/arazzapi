...
import getColumns from './Ref_RelationshipColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteRef_Relationship, getRef_Relationship} from "../../repo/ref_relationshipRepo";
/*
...
const Ref_RelationshipTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteRef_Relationship(oldData.code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Ref_Relationship Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getRef_Relationship(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/ref_relationship/update/${rowData.code}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/ref_relationship/add")
                }
            }
        ]}
...
export default withRouter(Ref_RelationshipTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Ref_RelationshipColumns';
