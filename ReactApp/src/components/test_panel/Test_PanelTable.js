...
import getColumns from './Test_PanelColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteTest_Panel, getTest_Panel} from "../../repo/test_panelRepo";
/*
...
const Test_PanelTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteTest_Panel(oldData.panel_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Test_Panel Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getTest_Panel(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/test_panel/update/${rowData.panel_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Test_PanelTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Test_PanelColumns';
