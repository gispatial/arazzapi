...
import getColumns from './Menu_ItemColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMenu_Item, getMenu_Item} from "../../repo/menu_itemRepo";
/*
...
const Menu_ItemTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteMenu_Item(oldData.item_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Menu_Item Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getMenu_Item(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/menu_item/update/${rowData.item_id}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Menu_ItemTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Menu_ItemColumns';
