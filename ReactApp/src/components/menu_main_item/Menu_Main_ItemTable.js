...
import getColumns from './Menu_Main_ItemColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteMenu_Main_Item, getMenu_Main_Item} from "../../repo/menu_main_itemRepo";
/*
...
const Menu_Main_ItemTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteMenu_Main_Item(oldData.main_id)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Menu_Main_Item Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getMenu_Main_Item(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/menu_main_item/update/${rowData.main_id}`,
                        user:rowData
                    })
                }
            },
            ...
            icon: ()=><AddBox variant="contained" color="secondary"/>,
                ...
                isFreeAction: true,
                onClick: (event, rowData) => {
                    history.push("/menu_main_item/add")
                }
            }
        ]}
...
export default withRouter(Menu_Main_ItemTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Menu_Main_ItemColumns';
