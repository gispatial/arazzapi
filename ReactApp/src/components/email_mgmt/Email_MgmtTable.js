...
import getColumns from './Email_MgmtColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteEmail_Mgmt, getEmail_Mgmt} from "../../repo/email_mgmtRepo";
/*
...
const Email_MgmtTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteEmail_Mgmt(oldData.code)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="Email_Mgmt Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getEmail_Mgmt(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/email_mgmt/update/${rowData.code}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(Email_MgmtTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './Email_MgmtColumns';
