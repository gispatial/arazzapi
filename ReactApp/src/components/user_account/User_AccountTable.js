...
import getColumns from './User_AccountColumns';
import Edit from "@material-ui/icons/Edit";
import {withRouter} from "react-router";
import {AddBox} from "@material-ui/icons";
import {deleteUser_Account, getUser_Account} from "../../repo/user_accountRepo";
/*
...
const User_AccountTable = (props) => {
    const history = props.history;
    const [columns, setColumns] = useState(getColumns({}));

    //Here we call delete
    const handleRowDelete = (oldData, resolve) => {
        deleteUser_Account(oldData.username)
            .then(res => {
                resolve()
            })
            .catch(error => {
    ...
    <MaterialTable
        minRows={20}
        title="User_Account Data"
        columns={columns}
        data={async(query)=>
            {
                const res = await getUser_Account(query.page,query.pageSize,query.search);
                return ({
                    data: res.records,
                    page: query.page,
        ...
        actions={[
            {
                ...
                onClick: (event,rowData) =>{
                    history.push({
                        pathname:`/user_account/update/${rowData.username}`,
                        user:rowData
                    })
                }
            },
...
export default withRouter(User_AccountTable);
import MaterialTable from 'material-table';
import React, {useEffect, useState} from 'react';
import tableIcons from '../templates/TableIcons';
import getColumns from './User_AccountColumns';
